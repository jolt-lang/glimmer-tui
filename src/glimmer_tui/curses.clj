...
(defn start!
  ...
  []
  ;; ncurses would exit the process here rather than return an error, which in a
  ;; REPL session means losing the session; refuse politely instead.
  (when (zero? (c/isatty 1))
